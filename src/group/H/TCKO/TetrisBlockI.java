...
public class TetrisBlockI extends TetrisBlock {
	...
	public boolean canMoveDown() {
		if (rotationPos == 0)
			...
			return blocks.get(0).canMove() && blocks.get(1).canMove() && canMove() && blocks.get(2).canMove();
		else
			return true;
	}
	...
	public void moveRight() {
		...
		for (TetrisBug tb : blocks)
			tb.setDirection(90);
		if (rotationPos == 0) {
			if (blocks.get(2).canMove() && canMove() && blocks.get(0).canMove() && blocks.get(1).canMove()) {
				move();
				blocks.get(2).move();
				blocks.get(0).move();
				blocks.get(1).move();
